"""Tier 2 Benchmarks: Node classification on real datasets.

Compares Quadratic, LorentzLinear (iid), LorentzLinear (ORF), and Hypformer
on Disease-NC, Cora, Citeseer, Airport.
"""
...
from data.loader import load_dataset, LOADERS
...
DATASET_HPARAMS = {
    "disease_nc": {"d_hyp": 16, "lr": 5e-3, "epochs": 300, "dropout": 0.3,
                   "M": 64, "n_layers": 2, "wd": 5e-4},
    "cora":       {"d_hyp": 16, "lr": 5e-3, "epochs": 300, "dropout": 0.5,
                   "M": 128, "n_layers": 2, "wd": 5e-4},
    "citeseer":   {"d_hyp": 16, "lr": 5e-3, "epochs": 300, "dropout": 0.5,
                   "M": 128, "n_layers": 2, "wd": 5e-4},
    "airport":    {"d_hyp": 16, "lr": 5e-3, "epochs": 300, "dropout": 0.3,
                   "M": 64, "n_layers": 2, "wd": 5e-4},
}
...
def _plot_results(all_results):
    ...
    fig.suptitle("Tier 2 Node Classification Benchmarks (5 seeds)", fontsize=13)
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, "tier2_benchmarks.png"), dpi=150)
    plt.close(fig)
    print(f"\n  Plot saved to {os.path.join(RESULTS_DIR, 'tier2_benchmarks.png')}")
